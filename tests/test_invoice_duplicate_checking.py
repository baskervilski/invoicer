...
def test_get_alternative_invoice_number_first_try(mock_settings, temp_invoice_dir):
    ...
    with patch('invoicer.main.settings', mock_settings):
        alternative = get_alternative_invoice_number(
            "TST", 
            "INV-202511-TST", 
            datetime(2025, 11, 1)
        )
...
def test_generate_invoice_number_format(mock_settings):
    ...
    with patch('invoicer.main.settings', mock_settings):
        invoice_number = generate_invoice_number(
            mock_settings.invoice_number_template,
            "TST",
            datetime(2025, 11, 15)
        )
...
def test_invoice_number_different_months(mock_settings):
    ...
    with patch('invoicer.main.settings', mock_settings):
        nov_number = generate_invoice_number(
            mock_settings.invoice_number_template,
            "TST",
            datetime(2025, 11, 1)
        )
        dec_number = generate_invoice_number(
            mock_settings.invoice_number_template,
            "TST", 
            datetime(2025, 12, 1)
        )
...
def test_invoice_number_different_clients(mock_settings):
    ...
    with patch('invoicer.main.settings', mock_settings):
        client1_number = generate_invoice_number(
            mock_settings.invoice_number_template,
            "CLI1",
            test_date
        )
        client2_number = generate_invoice_number(
            mock_settings.invoice_number_template,
            "CLI2", 
            test_date
        )
