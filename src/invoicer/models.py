...
VatNumberField = Annotated[
    str, BeforeValidator(strip_whitespace), Field(max_length=50, description="VAT number for the client")
]

ClientEmailField = Annotated[EmailStr, Field(description="Client email address for invoice delivery")]


# ID and identifier fields used only in models
UniqueIdField = Annotated[str, Field(description="Unique identifier")]

AutoGeneratedIdField = Annotated[
    str, Field(default_factory=generate_uuid, description="Auto-generated unique identifier")
]
...
UnitTypeField = Annotated[
    str, BeforeValidator(strip_whitespace), Field(default="days", description="Unit type (days, hours, items, etc.)")
]
...
QuantityField = Annotated[float, Field(gt=0, description="Quantity (days, hours, items, etc.)")]

RateField = Annotated[float, Field(gt=0, description="Rate per unit")]

AmountField = Annotated[float, Field(ge=0, description="Monetary amount")]

TaxRateField = Annotated[float, Field(ge=0, le=1, description="Tax rate as decimal (0.0 to 1.0)")]

TotalAmountField = Annotated[float, Field(ge=0, description="Total amount due")]

InvoiceCountField = Annotated[int, Field(default=0, ge=0, description="Number of invoices")]

DaysWorkedField = Annotated[Optional[int], Field(default=None, ge=0, description="Number of days worked")]

NonNegativeFloatField = Annotated[float, Field(ge=0, description="Non-negative floating point number")]

SubtotalField = Annotated[float, Field(ge=0, description="Subtotal amount before tax")]

TaxAmountField = Annotated[float, Field(ge=0, description="Tax amount in currency")]

# Date fields used only in models
AutoDateTimeField = Annotated[datetime, Field(default_factory=datetime.now, description="Auto-generated timestamp")]

OptionalDateTimeField = Annotated[Optional[datetime], Field(default=None, description="Optional date and time")]

InvoiceDateField = Annotated[datetime, Field(default_factory=datetime.now, description="Invoice date")]

DueDateField = Annotated[str, Field(default="Net 30 days", description="Due date description")]

# Contact fields used only in models
PhoneField = Annotated[Optional[str], Field(default="", description="Phone number with international format support")]

# List fields used only in models
ProjectIdListField = Annotated[
    list[str], Field(default_factory=list, description="List of project IDs associated with this entity")
]
...
class InvoiceItemModel(BaseModel):
    ...
    def validate_amount_matches_calculation(cls, v, info):
        ...
        if "quantity" in values and "rate" in values:
            ...
            if abs(v - expected_amount) > 0.01:  # Allow for small floating point differences
                raise ValueError(
...
class InvoiceModel(BaseModel):
    ...
    def validate_tax_amount(cls, v, info):
        ...
        if "subtotal" in values and "tax_rate" in values:
            ...
            if abs(v - expected_tax) > 0.01:  # Allow for small floating point differences
                raise ValueError(
    ...
    def validate_subtotal_matches_line_items(cls, v, info):
        ...
        if "line_items" in values and values["line_items"]:
            ...
            if abs(v - expected_subtotal) > 0.01:  # Allow for small floating point differences
                raise ValueError(f"Subtotal {v} doesn't match sum of line items {expected_subtotal}")
        return round(v, 2)
